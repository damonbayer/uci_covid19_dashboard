...
icu_plot <- icu_plot_data %>% 
  ...
  ggtitle(glue("ICU Patients with COVID-19"),
...
hospitalizations_plot <- hospitalizations_plot_data %>% 
  ggplot(aes(date, value / population * per_n_people, group = county, color = county)) +
  ggtitle(glue("Hospitalized Patients with COVID-19"),
          subtitle = glue("{ma_n} Day Moving Average")) +
  gglayers +
...
deaths_plot <- deaths_plot_data %>% 
  ggplot(aes(date, value / population * per_n_people, group = county, color = county)) +
  ggtitle(glue(" New Daily Deaths due to COVID-19"),
          subtitle = glue("{ma_n} Day Moving Average")) +
  gglayers +
  ylab(glue("Daily Deaths due to COVID-19\nper {comma(per_n_people)} People")) +
  annotate("rect",
...
cases_plot <- cases_plot_data %>% 
  ggplot(aes(date, value / population * per_n_people, group = county, color = county)) +
  ggtitle(glue("New Daily Confirmed COVID-19 Cases"),
          subtitle = glue("{ma_n} Day Moving Average")) +
  gglayers +
  ylab(glue("Daily Confirmed COVID-19 Cases\nper {comma(per_n_people)} People")) +
    annotate("rect",
